...
CosineLSHRecommendation::CosineLSHRecommendation(const std::vector<Tweet>& tweets, unsigned int neighborsArg, int kLSH, int L)
   		: Recommendation(tweets), numberOfNeighbors(neighborsArg), lsh(NULL) {
	// Insert every user vector in the Cosine LSH
	lsh = new LSH(kLSH, userSentiments[0].getDimensions(), L, userSentiments.size());
	for (unsigned int i = 0; i < userSentiments.size(); i++) {
		lsh->insert(userSentiments[i]);
	}
}
...
std::vector< std::vector<unsigned int> > CosineLSHRecommendation::recommendations() const {
	...
	for (unsigned int i = 0; i < userSentiments.size(); i++) {
		...
		lsh->findAllNeighbors(userSentiments[i], 0.0, neighbors, distances);
